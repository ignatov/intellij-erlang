bind_pat_vars([Pat|PatLeft], [Type|TypeLeft], Acc, Map, State, Rev) ->
  ...
  {NewMap, TypeOut} =
	...
	LiteralOrOpaque =
	  case t_opaque_match_atom(Literal, State#<error>state</error>.opaques) of
	    [Opaque] -> Opaque;
	    _ -> Literal
	  end,
	...
	{Map1, t_product(EsTypes)};
      var ->
	Opaques = State#<error>state</error>.opaques,
	VarType1 =
...
bind_opaque_pats(GenType, Type, Pat, Map, State, Rev) ->
  case t_find_opaque_mismatch(GenType, Type) of
    {ok, T1, T2}  ->
      case lists:member(T2, State#<error>state</error>.opaques) of
	true ->
...
handle_guard_gen_fun({M, F, A}, Guard, Map, Env, Eval, State) ->
  ...
  MapFun = fun(Type) ->
	       case lists:member(Type, State#<error>state</error>.opaques) of
...
bind_type_test(Eval, TypeTest, ArgType, State) ->
  ...
  Mode = determine_mode(ArgType, State#<error>state</error>.opaques),
  case Eval of
    pos ->
...
handle_guard_is_record(Guard, Map, Env, Eval, State) ->
  ...
  Mode = determine_mode(RecType, State#<error>state</error>.opaques),
  NewTupleType =
    case t_opaque_match_record(TupleType, State#<error>state</error>.opaques) of
      [Opaque] -> Opaque;
      _ -> TupleType
    end,
...
state__new(Callgraph, Tree, Plt, Module, Records, BehaviourTranslations) ->
  ...
  #<error>state</error>{callgraph = Callgraph, envs = Env, fun_tab = FunTab, opaques = Opaques,
	 plt = Plt, races = dialyzer_races:new(), records = Records,
	 warning_mode = false, warnings = [], work = Work, tree_map = TreeMap,
	 module = Module, behaviour_api_dict = BehaviourTranslations}.

state__mark_fun_as_handled(#<error>state</error>{fun_tab = FunTab} = State, Fun0) ->
  Fun = get_label(Fun0),
  case dict:find(Fun, FunTab) of
    {ok, {not_handled, Entry}} ->
      State#<error>state</error>{fun_tab = dict:store(Fun, Entry, FunTab)};
    {ok, {_, _}} ->
      State
  end.

state__warning_mode(#<error>state</error>{warning_mode = WM}) ->
  WM.

state__set_warning_mode(#<error>state</error>{tree_map = TreeMap, fun_tab = FunTab,
                               races = Races} = State) ->
  ?debug("Starting warning pass\n", []),
  Funs = dict:fetch_keys(TreeMap),
  State#<error>state</error>{work = init_work([top|Funs--[top]]),
	      fun_tab = FunTab, warning_mode = true,
              races = dialyzer_races:put_race_analysis(true, Races)}.

state__restore_race_code(RaceCode, #<error>state</error>{callgraph = Callgraph} = State) ->
  State#<error>state</error>{callgraph = dialyzer_callgraph:put_race_code(RaceCode,
                                                           Callgraph)}.

state__race_analysis(Analysis, #<error>state</error>{races = Races} = State) ->
  State#<error>state</error>{races = dialyzer_races:put_race_analysis(Analysis, Races)}.

state__renew_curr_fun(CurrFun, CurrFunLabel,
                      #<error>state</error>{races = Races} = State) ->
  State#<error>state</error>{races = dialyzer_races:put_curr_fun(CurrFun, CurrFunLabel,
                                                  Races)}.

state__renew_fun_args(Args, #<error>state</error>{races = Races} = State) ->
  case state__warning_mode(State) of
    true -> State;
    false ->
      State#<error>state</error>{races = dialyzer_races:put_fun_args(Args, Races)}
  end.

state__renew_race_list(RaceList, RaceListSize,
                       #<error>state</error>{races = Races} = State) ->
  State#<error>state</error>{races = dialyzer_races:put_race_list(RaceList, RaceListSize,
                                                   Races)}.

state__renew_warnings(Warnings, State) ->
  State#<error>state</error>{warnings = Warnings}.

-spec state__add_warning(dial_warning(), state()) -> state().

state__add_warning(Warn, #<error>state</error>{warnings = Warnings} = State) ->
  State#<error>state</error>{warnings = [Warn|Warnings]}.

state__add_warning(State, Tag, Tree, Msg) ->
  state__add_warning(State, Tag, Tree, Msg, false).

state__add_warning(#<error>state</error>{warning_mode = false} = State, _, _, _, _) ->
  State;
state__add_warning(#<error>state</error>{warnings = Warnings, warning_mode = true} = State,
		   Tag, Tree, Msg, Force) ->
...
state__get_race_warnings(#<error>state</error>{races = Races} = State) ->
  {Races1, State1} = dialyzer_races:get_race_warnings(Races, State),
  State1#<error>state</error>{races = Races1}.

state__get_warnings(#<error>state</error>{tree_map = TreeMap, fun_tab = FunTab,
  ...
  #<error>state</error>{warnings = Warn} = lists:foldl(FoldFun, State, dict:to_list(TreeMap)),
  Warn.

state__is_escaping(Fun, #<error>state</error>{callgraph = Callgraph}) ->
  dialyzer_callgraph:is_escaping(Fun, Callgraph).

state__lookup_type_for_rec_var(Var, #<error>state</error>{callgraph = Callgraph} = State) ->
  Label = get_label(Var),
...
state__lookup_name({_, _, _} = MFA, #<error>state</error>{}) ->
  MFA;
state__lookup_name(top, #<error>state</error>{}) ->
  top;
state__lookup_name(Fun, #<error>state</error>{callgraph = Callgraph}) ->
  case dialyzer_callgraph:lookup_name(Fun, Callgraph) of
    {ok, MFA} -> MFA;
    error -> Fun
  end.

state__lookup_record(Tag, Arity, #<error>state</error>{records = Records}) ->
...
state__get_args(Tree, #<error>state</error>{fun_tab = FunTab}) ->
  Fun = get_label(Tree),
  case dict:find(Fun, FunTab) of
...
state__update_fun_env(Tree, Map, #<error>state</error>{envs = Envs} = State) ->
  NewEnvs = dict:store(get_label(Tree), Map, Envs),
  State#<error>state</error>{envs = NewEnvs}.

state__fun_env(Tree, #<error>state</error>{envs = Envs}) ->
  Fun = get_label(Tree),
...
state__clean_not_called(#<error>state</error>{fun_tab = FunTab} = State) ->
  NewFunTab =
    dict:map(fun(top, Entry) -> Entry;
		(_Fun, {not_handled, {Args, _}}) -> {Args, t_none()};
		(_Fun, Entry) -> Entry
	     end, FunTab),
  State#<error>state</error>{fun_tab = NewFunTab}.

state__all_fun_types(#<error>state</error>{fun_tab = FunTab}) ->
  Tab1 = dict:erase(top, FunTab),
  dict:map(fun(_Fun, {Args, Ret}) -> t_fun(Args, Ret)end, Tab1).

state__fun_type(Fun, #<error>state</error>{fun_tab = FunTab}) ->
  Label =
...
state__update_fun_entry(Tree, ArgTypes, Out0,
			#<error>state</error>{fun_tab=FunTab, callgraph=CG, plt=Plt} = State)->
  Fun = get_label(Tree),
  Out1 =
    if Fun =:= top -> Out0;
       true ->
  ...
  case dict:find(Fun, FunTab) of
	...
	false ->
	  ...
	  State1 = State#<error>state</error>{fun_tab = NewFunTab},
...
state__add_work_from_fun(_Tree, #<error>state</error>{warning_mode = true} = State) ->
  State;
state__add_work_from_fun(Tree, #<error>state</error>{callgraph = Callgraph,
...
state__add_work(top, State) ->
  State;
state__add_work(Fun, #<error>state</error>{work = Work} = State) ->
  NewWork = add_work(Fun, Work),
  State#<error>state</error>{work = NewWork}.

state__get_work(#<error>state</error>{work = Work, tree_map = TreeMap} = State) ->
  case get_work(Work) of
    none -> none;
    {Fun, NewWork} ->
      {dict:fetch(Fun, TreeMap), State#<error>state</error>{work = NewWork}}
  end.

state__lookup_call_site(Tree, #<error>state</error>{callgraph = Callgraph}) ->
  Label = get_label(Tree),
  dialyzer_callgraph:lookup_call_site(Label, Callgraph).

state__fun_info(external, #<error>state</error>{}) ->
  external;
state__fun_info({_, _, _} = MFA, #<error>state</error>{plt = PLT}) ->
  {MFA,
   dialyzer_plt:lookup(PLT, MFA),
   dialyzer_plt:lookup_contract(PLT, MFA),
   t_any()};
state__fun_info(Fun, #<error>state</error>{callgraph = CG, fun_tab = FunTab, plt = PLT}) ->
  {Sig, Contract} =
...
state__find_apply_return(Tree, #<error>state</error>{callgraph = Callgraph} = State) ->
...
forward_args(Fun, ArgTypes, #<error>state</error>{work = Work, fun_tab = FunTab} = State) ->
  ...
  case Fixpoint of
    ...
    false ->
      ...
      State#<error>state</error>{work = NewWork, fun_tab = NewFunTab}
  end.

-spec state__cleanup(state()) -> state().

state__cleanup(#<error>state</error>{callgraph = Callgraph,
                      races = Races,
                      records = Records}) ->
  #<error>state</error>{callgraph = dialyzer_callgraph:cleanup(Callgraph),
         races = dialyzer_races:cleanup(Races),
         records = Records}.

-spec state__get_callgraph(state()) -> dialyzer_callgraph:callgraph().

state__get_callgraph(#<error>state</error>{callgraph = Callgraph}) ->
  Callgraph.

-spec state__get_races(state()) -> dialyzer_races:races().

state__get_races(#<error>state</error>{races = Races}) ->
  Races.

-spec state__get_records(state()) -> dict().

state__get_records(#<error>state</error>{records = Records}) ->
  Records.
...
state__put_callgraph(Callgraph, State) ->
  State#<error>state</error>{callgraph = Callgraph}.

-spec state__put_races(dialyzer_races:races(), state()) -> state().

state__put_races(Races, State) ->
  State#<error>state</error>{races = Races}.

-spec state__records_only(state()) -> state().

state__records_only(#<error>state</error>{records = Records}) ->
  #<error>state</error>{records = Records}.
