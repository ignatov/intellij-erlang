...
lock_record(Tid, Ts, Tab, Key, LockKind) when is_atom(Tab) ->
    Store = Ts#<error>tidstore</error>.store,
    Oid =  {Tab, Key},
    case LockKind of
	read ->
...
stored_keys(Tab,'$end_of_table',Prev,Ts,Op,Type) ->
    case ts_keys(Ts#<error>tidstore</error>.store,Tab,Op,Type,[]) of
	[] -> '$end_of_table';
...
stored_keys(Tab,{'EXIT',{aborted,R={badarg,[Tab,Key]}}},
	    Key,#<error>tidstore</error>{store=Store},Op,Type) ->
...
stored_keys(_,{'EXIT',{aborted,R}},_,_,_,_) ->
    abort(R);
stored_keys(Tab,Key,Prev,#<error>tidstore</error>{store=Store},Op,ordered_set) ->
    case ?ets_match(Store, {{Tab, Key}, '_', '$1'}) of
	[] -> 
...
stored_keys(Tab,Key,_,#<error>tidstore</error>{store=Store},Op,_) ->
    case ?ets_match(Store, {{Tab, Key}, '_', '$1'}) of
	[] ->  Key;
 	Ops ->
	    case lists:last(Ops) of
...
add_previous(_Tid, Ts, _Type, Tab) ->
    Previous = ?ets_match(Ts#<error>tidstore</error>.store, {{Tab, '$1'}, '_', write}),
...
match_object(Tid, Ts, Tab, Pat, LockKind) 
  when is_atom(Tab), Tab /= schema, is_tuple(Pat), tuple_size(Pat) > 2 ->
	...
	tid ->
	    ...
	    add_written_match(Ts#<error>tidstore</error>.store, Pat, Tab, Objs);
	_Protocol ->
...
snmp_get_row(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
    case get(mnesia_activity_state) of
 	{Mod, Tid, Ts=#<error>tidstore</error>{store=Store}} when element(1, Tid) =:= tid ->
...
snmp_get_next_index(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
    ...
    case get(mnesia_activity_state) of
 	{_Mod, Tid, #<error>tidstore</error>{store=Store}} when element(1, Tid) =:= tid ->
	    case OrigKey of
		undefined ->
...
snmp_get_mnesia_key(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
    case get(mnesia_activity_state) of
 	{_Mod, Tid, Ts} when element(1, Tid) =:= tid ->
	    ...
	    snmp_filter_key(Res, RowIndex, Tab, Ts#<error>tidstore</error>.store);
	_ -> 
...
put_activity_id(Activity) -> mnesia_tm:put_activity_id(Activity).
put_activity_id(Activity,Fun) -> mnesia_tm:put_activity_id(Activity,Fun).