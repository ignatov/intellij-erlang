...
public class ErlangTestFinder implements TestFinder {
  ...
  public Collection<PsiElement> findTestsForClass(@NotNull PsiElement element) {
    ...
    for (String suffix : SUFFIXES) {
      var virtualFiles = FilenameIndex.getVirtualFilesByName(virtualFile.getNameWithoutExtension() + suffix + EXT, getScope(project));
      var psiFiles = ContainerUtil.mapNotNull(virtualFiles, pm::findFile);
      result.addAll(psiFiles);
    }
    return result;
  }
  ...
  public Collection<PsiElement> findClassesForTest(@NotNull PsiElement element) {
    ...
    for (String suffix : SUFFIXES) {
      if (name.endsWith(suffix)) {
        var virtualFiles = FilenameIndex.getVirtualFilesByName(name.substring(0, length - suffix.length()) + EXT, getScope(project));
        var psiFiles = ContainerUtil.mapNotNull(virtualFiles, pm::findFile);
        result.addAll(psiFiles);
      }
    }
